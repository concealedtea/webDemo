...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		var bottommsg=typeof this.settings.bottom_msgtip =="undefined" || this.settings.bottom_msgtip.length==0 ?"":"<span class='file-msg-botton'>"+this.settings.bottom_msgtip +"</span>";
		this.$element.after('<span class="ace-file-container" data-title="'+this.settings.btn_choose+'"><span class="ace-file-name" data-title="'+this.settings.no_file+'">'+(this.settings.no_icon ? '<i class="'+ ace.vars['icon'] + this.settings.no_icon+'"></i>' : '')+bottommsg+'</span></span>');
	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			var dataFileName=this.settings.showFileName?filename:"";
			if(!this.well_style) this.$label.find('.ace-file-name').attr({'data-title':dataFileName}).find(ace.vars['.icon']).attr('class', ace.vars['icon'] + fileIcon);
				...
				var bottommsg=typeof this.settings.bottom_msgtip =="undefined" || this.settings.bottom_msgtip.length==0 ?"":"<span class='file-msg-botton'>"+this.settings.bottom_msgtip +"</span>";
				this.$label.append('<span class="ace-file-name" data-title="'+dataFileName+'"><i class="'+ ace.vars['icon'] + fileIcon+'"></i>'+bottommsg+'</span>');
	...
	var handle_on_change = function() {
		...
		if(this.settings.ajaxCallback){
			$.when(checkFileWh.call(this, file_list[0])).done(function(result){
				if(result.success){
	...
	var checkFileWh = function(file) {
		...
		var checkCallback=this.settings.checkCallback;
	...
	var initPreviewImage = function(self,srcurl) {
		//var self = this;
		self.$label.addClass('hide-placeholder');
		self.$label.removeAttr("data-title").addClass('selected');
		...
		var imgLoaded = function(img) {
			...
			if(thumb.src.length==0){
				//thumb.src=img.src;
				$(img).css({'background-image':'url('+img.src+')' , width:w, height:h}).attr({src:img.src}).show();
			}else{
				$(img).css({'background-image':'url('+thumb.src+')' , width:w, height:h}).attr({src:'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg=='}).show();
			}
			///////////////////
			deferred.resolve();
		};
		var imgFailed = function(img) {
			...
			$span.find(".file-msg-botton").remove();
	...
	var checkFileList = function(files, dropped) {
		...
		if(error_count.size>0)arrErrMsg.push("上传图片大小不正确");
